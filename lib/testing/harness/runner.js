...
const couponTargetBody = (target) =>
  target.kind === "plan" ? { planId: target.id } : { topupId: target.id };

/** Keep not-found and evaluated responses on one documented SDK shape. */
const couponValidation = (result) => ({
  ...
  durationInMonths: result?.durationInMonths ?? null,
...
const rx = {
  ...
  /**
   * Purchasable plans and topups, with per-user eligibility when a user is
   * given. `platform: "ios"` quotes App Store prices where they differ.
   */
  catalog: (rxlabUserId, options = {}) =>
    apiFetch(`/api/v1/catalog${query({ rxlabUserId, platform: options.platform })}`),
  ...
  coupons: {
    validate: async (rxlabUserId, code, target) =>
      couponValidation(
        await apiFetch("/api/v1/coupons/validate", {
          method: "POST",
          body: JSON.stringify({
            rxlabUserId,
            code,
            ...couponTargetBody(target),
          }),
        }),
      ),
    reserve: (rxlabUserId, code, target) =>
      control("coupon.reserve", { rxlabUserId, code, target }),
  },
  ...
  config: {
    ...
    coupons: () => control("config.coupons"),
  ...
  testUsers: {
    ...
    grantPlan: (rxlabUserId, planKey, options = {}) =>
      control("testUser.grantPlan", {
        rxlabUserId,
        planKey,
    ...
    setTime: (rxlabUserId, at) => {
      const targetMs = at instanceof Date ? at.getTime() : new Date(at).getTime();
      if (!Number.isFinite(targetMs)) {
        throw new TypeError("at must be a valid timestamp, epoch milliseconds, or Date");
      }
      return control("testUser.setClock", {
        rxlabUserId,
        offsetMs: Math.trunc(targetMs - Date.now()),
      });
    },
